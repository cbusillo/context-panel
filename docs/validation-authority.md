...
`macos.widget` is rendered on the host by a SwiftPM tool the executor builds from
the verified source root, with no app or extension bundle. `macos.app` remains
an explicit `unsupported-host-mechanism` result;
