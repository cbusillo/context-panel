...
companion builds, and the CodeQL analysis, when none of its changed paths can
affect them. Unknown paths run everything, and every push to `main` runs the
full set. When adding a build input outside `Sources/`, `Tools/`, and `Config/`
