...
struct ProbeConfiguration {
    ...
    private static func printHelp() {
        print("""
        Usage: swift run ClaudeLimitProbe [--stats PATH] [--rate-limit-cache PATH]

        Prints a redacted Claude local status summary. Pass --stats to include
        session counts from ~/.claude/stats-cache.json (optional; omitting avoids
        reading Claude Code's data directory). This probe intentionally does not
        read Keychain secrets, token files, raw transcripts, emails, account IDs,
        org IDs, or provider response bodies.
        """)
    }
}
...
struct ClaudeLimitProbe {
    ...
    private static func printSummary(result: ConnectorRefreshResult, statsPath: String?, rateLimitCachePath: String) {
        ...
        print("stats cache: \(statsPath.map(ConnectorRedactor.redactedPath) ?? "not configured")")
