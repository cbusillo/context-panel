...
write_local_oauth_xcconfig() {
	mkdir -p "$(dirname "$local_oauth_xcconfig_path")"
	{
		...
		printf 'CONTEXT_PANEL_GOOGLE_OAUTH_CLIENT_ID = %s\n' \
			"$(shell_escape_xcconfig_value "${CONTEXT_PANEL_GOOGLE_OAUTH_CLIENT_ID:-}")"
		printf 'CONTEXT_PANEL_GOOGLE_OAUTH_CLIENT_SECRET = %s\n' \
			"$(shell_escape_xcconfig_value "${CONTEXT_PANEL_GOOGLE_OAUTH_CLIENT_SECRET:-}")"
...
check_debug_google_oauth_config() {
	...
	client_id="$(normalize_plist_build_setting_value "$(plist_value "$plist" CONTEXT_PANEL_GOOGLE_OAUTH_CLIENT_ID)")"
	client_secret="$(normalize_plist_build_setting_value "$(plist_value "$plist" CONTEXT_PANEL_GOOGLE_OAUTH_CLIENT_SECRET)")"
