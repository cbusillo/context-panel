...
context_files() {
	...
	for root in "${roots[@]}"; do
		...
		find "$root" -maxdepth 12 \
			\( -name accounts.json -o -name file-bookmarks.json -o -name current-snapshot.json -o -name history.json -o -name reset-primer-settings.json -o -name reset-primer-runs.json -o -name background-refresh-settings.json -o -name limit-warning-settings.json -o -name limit-warning-state.json -o -name webhook-settings.json -o -name webhook-delivery-state.json \) \
