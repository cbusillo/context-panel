...
Local debug Google OAuth config:
            For install/reset, set CONTEXT_PANEL_GOOGLE_OAUTH_CLIENT_ID in the
            environment, or create .local/context-panel-runtime.env. Use a Google
            Apple/iOS installed-app OAuth client ID. The callback scheme is
            derived as com.googleusercontent.apps.<client-id-prefix> unless
            CONTEXT_PANEL_GOOGLE_OAUTH_CALLBACK_SCHEME is set. Client secret is
            optional for native clients. Legacy client values can be supplied to
            keep older web-client refresh tokens working. The local env file is
...
redact_build_output() {
	if [[ -z "${CONTEXT_PANEL_GOOGLE_OAUTH_CLIENT_ID:-}" && -z "${CONTEXT_PANEL_GOOGLE_OAUTH_CLIENT_SECRET:-}" && -z "${CONTEXT_PANEL_GOOGLE_OAUTH_LEGACY_CLIENT_ID:-}" && -z "${CONTEXT_PANEL_GOOGLE_OAUTH_LEGACY_CLIENT_SECRET:-}" ]]; then
		cat
	else
		CLIENT_ID="${CONTEXT_PANEL_GOOGLE_OAUTH_CLIENT_ID:-}" \
			CLIENT_SECRET="${CONTEXT_PANEL_GOOGLE_OAUTH_CLIENT_SECRET:-}" \
			LEGACY_CLIENT_ID="${CONTEXT_PANEL_GOOGLE_OAUTH_LEGACY_CLIENT_ID:-}" \
			LEGACY_CLIENT_SECRET="${CONTEXT_PANEL_GOOGLE_OAUTH_LEGACY_CLIENT_SECRET:-}" \
			perl -pe '
                for my $secret ($ENV{CLIENT_ID}, $ENV{CLIENT_SECRET}, $ENV{LEGACY_CLIENT_ID}, $ENV{LEGACY_CLIENT_SECRET}) {
...
load_local_runtime_env() {
	...
	while IFS= read -r line || [[ -n "$line" ]]; do
		...
		case "$line" in
		CONTEXT_PANEL_GOOGLE_OAUTH_CLIENT_ID=* | CONTEXT_PANEL_GOOGLE_OAUTH_CLIENT_SECRET=* | CONTEXT_PANEL_GOOGLE_OAUTH_CALLBACK_SCHEME=* | CONTEXT_PANEL_GOOGLE_OAUTH_LEGACY_CLIENT_ID=* | CONTEXT_PANEL_GOOGLE_OAUTH_LEGACY_CLIENT_SECRET=*) ;;
		*) continue ;;
		esac
		key="${line%%=*}"
		value="${line#*=}"
...
write_local_oauth_xcconfig() {
	...
	local callback_scheme="${CONTEXT_PANEL_GOOGLE_OAUTH_CALLBACK_SCHEME:-}"
	if [[ -z "$callback_scheme" && -n "${CONTEXT_PANEL_GOOGLE_OAUTH_CLIENT_ID:-}" ]]; then
		callback_scheme="$(google_oauth_callback_scheme_for_client_id "${CONTEXT_PANEL_GOOGLE_OAUTH_CLIENT_ID}")"
	fi
	{
		...
		printf 'CONTEXT_PANEL_GOOGLE_OAUTH_CALLBACK_SCHEME = %s\n' \
			"$(xcconfig_literal_value "$callback_scheme")"
		printf 'CONTEXT_PANEL_GOOGLE_OAUTH_LEGACY_CLIENT_ID = %s\n' \
			"$(xcconfig_literal_value "${CONTEXT_PANEL_GOOGLE_OAUTH_LEGACY_CLIENT_ID:-}")"
		printf 'CONTEXT_PANEL_GOOGLE_OAUTH_LEGACY_CLIENT_SECRET = %s\n' \
			"$(xcconfig_literal_value "${CONTEXT_PANEL_GOOGLE_OAUTH_LEGACY_CLIENT_SECRET:-}")"
	} >"$local_oauth_xcconfig_path"
}

report_local_google_oauth_config() {
	if [[ -n "${CONTEXT_PANEL_GOOGLE_OAUTH_CLIENT_ID:-}" ]]; then
...
require_local_google_oauth_config() {
	...
	fail "debug Google OAuth build settings are required for install/reset; set CONTEXT_PANEL_GOOGLE_OAUTH_CLIENT_ID or create $local_runtime_env_path"
	return 1
}
...
check_debug_google_oauth_config() {
	local plist="$1"
	local label="$2"
	local requires_callback_scheme="${3:-false}"
	if [[ ! -f "$plist" ]]; then
		fail "$label Info.plist is missing for Google OAuth config check"
		return 0
	fi
	local raw_client_id raw_client_secret raw_callback_scheme client_id client_secret callback_scheme expected_callback_scheme
	...
	raw_callback_scheme="$(plist_value "$plist" CONTEXT_PANEL_GOOGLE_OAUTH_CALLBACK_SCHEME)"
	...
	callback_scheme="$(normalize_plist_build_setting_value "$raw_callback_scheme")"
	expected_callback_scheme="$(google_oauth_callback_scheme_for_client_id "$client_id")"
	...
	if [[ -n "$callback_scheme" && "$callback_scheme" != \$\(* && "$raw_callback_scheme" != '""' ]] && has_wrapping_quotes "$raw_callback_scheme"; then
		fail "$label embeds a quoted Google OAuth callback scheme; runtime would send the quotes"
	fi
	if [[ "$requires_callback_scheme" == "true" && -n "$client_id" && -n "$expected_callback_scheme" && "$callback_scheme" != "$expected_callback_scheme" ]]; then
		fail "$label does not register the derived Google OAuth callback scheme $expected_callback_scheme"
	fi
	if [[ -n "$client_id" ]]; then
...
check_runtime() {
	...
	check_debug_google_oauth_config "$app_path/Contents/Info.plist" "app" true
