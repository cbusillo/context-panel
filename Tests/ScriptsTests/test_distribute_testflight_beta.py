...
class TestFlightDistributionTests(unittest.TestCase):
    ...
    def test_all_builds_group_needs_no_explicit_membership(self):
        client = FakeASCClient()
        group = {
            "id": "group-internal",
            "attributes": {
        ...
        result = distribute_testflight_beta.add_build_to_group(client, build, group, False)

        self.assertEqual(result, "already-present")
        self.assertEqual(client.requests, [])

    def test_all_builds_group_dry_run_needs_no_explicit_membership(self):
        client = FakeASCClient()
        group = {
            "id": "group-internal",
            "attributes": {
        ...
        result = distribute_testflight_beta.add_build_to_group(client, build, group, True)
